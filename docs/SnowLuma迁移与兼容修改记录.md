...
- Docker 容器：
  - `snowluma`：官方镜像 `motricseven7/snowluma:latest`，端口映射
    `6081(noVNC) / 5099(WebUI) / 3000-3001(预留) / 8084(OneBot ws)`
    启动参数必须保留：`--shm-size=1g --cap-add=SYS_PTRACE --security-opt seccomp=unconfined`
  ...
  - 旧 NapCat 容器：`3860863656-8082-6102`（在用）、`3988941800-8084`（已停，待删）、
    `3821805584-8080`、`1319279034-8083`（已停）
...
3. **第二个号迁移**：花晟九(3860863656) 迁移方案已定未执行——重建 SnowLuma 容器时加
   `SNOWLUMA_EXTRA_QQ_HOMES=/app/qq-acct2` + 独立卷 `qq-client-account-2:/app/qq-acct2`
   + `-p 8086:8086` + `--shm-size=2g`，扫码后配置 ws 服务端 0.0.0.0:8086
   （token 与 java 配置一致即可，java 端不用改）。
4. **停用账号**：8080(3821805584)/8083(1319279034) 两个 NapCat 容器已停，确认不要后可
   `docker rm` 并注释掉 java 配置里对应的 bot 条目（否则日志持续刷重试失败）。
5. **清理**：确认稳定后删除旧容器 `3988941800-8084`；宿主机 snowluma 的 systemd 服务可删。
