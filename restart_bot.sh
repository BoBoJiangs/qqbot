...
log "清理文件锁..."
(command -v lsof >/dev/null 2>&1 && (command -v lsof >/dev/null 2>&1 && (command -v lsof >/dev/null 2>&1 && lsof -t "$SCRIPT_DIR/bot.jar" > /tmp/bot.lock.pids 2>/dev/null) || : > /tmp/bot.lock.pids) || : > /tmp/bot.lock.pids) || : > /tmp/bot.lock.pids
